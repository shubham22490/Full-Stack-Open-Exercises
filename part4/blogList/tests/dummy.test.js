...
describe('total likes', () => {
  ...
  test('when list has only one blog, equals zero likes', () => {
    const result = listHelper.totalLikes([blogs[4]])
    assert.strictEqual(result, 0)
  })

  test('testing likes of multiple blogs', () => {
...
describe('favorite blogs', () => {
    ...
    test ('list of multiple blogs', () => {
        const res = listHelper.favoriteBlog(blogs)
        assert.deepStrictEqual(res, blogs[2])
    })

    test('list containing hihgest like muliple blogs', () => {
        const res = listHelper.favoriteBlog([blogs[0], blogs[1]])
        assert.deepStrictEqual(res,blogs[0] || blogs[1])
    })
})